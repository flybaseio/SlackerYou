	...
	var messagesRef = new Flybase("YOUR-API-KEY", "slacker", "chat");
	...
	messagesRef.limit(25).orderBy({"_id":-1}).on('value', function (snapshot) {
		if( snapshot.count() ){
			snapshot.forEach( function(row){
				addMessage( row.value() );
			});
		}
	});
	messagesRef.limitToLast(10).on('added', function (snapshot) {
		// Get data from returned
		addMessage(snapshot.value());
	});
